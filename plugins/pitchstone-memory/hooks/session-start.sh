...
- \`Memory/Projects/${project}/decisions.md\` and \`gotchas.md\` sit beside it,
  append-only and dated. They are history: read them when a question calls for
  one, and where they contradict \`state.md\`, \`state.md\` is right.
